...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-md">
      ...
      <div className="hidden border-b border-slate-100 bg-slate-900 px-4 py-1 text-[11px] text-slate-300 sm:block">
      ...
      <div className="mx-auto flex max-w-[1536px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-8">
          ...
          >
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-900 text-white shadow-sm ring-1 ring-slate-900/20 transition group-hover:bg-slate-800 group-hover:scale-105">
              <svg
                viewBox="0 0 24 24"
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1 leading-none">
                ...
                <span className="ml-1 rounded bg-slate-100 px-1 py-0.5 text-[9px] font-bold uppercase tracking-wider text-slate-600 font-mono border border-slate-200">
                  v2.4
                </span>
              </div>
              <span className="text-[9px] font-medium tracking-wide text-slate-600 leading-none mt-0.5">
          ...
          <nav className="hidden items-center gap-1 md:flex">
            {links.map((link) => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `rounded-md px-3 py-1.5 text-xs font-semibold tracking-wide transition ${
                    isActive
        ...
        <form onSubmit={handleSearchSubmit} className="flex-1 max-w-md hidden sm:block">
          <div className="relative flex items-center rounded-lg border border-slate-200 bg-slate-50/80 px-3 py-1.5 text-slate-800 transition focus-within:border-slate-900 focus-within:bg-white focus-within:ring-2 focus-within:ring-slate-900/5">
            ...
            />
            <kbd className="hidden lg:inline-flex items-center gap-0.5 rounded border border-slate-200 bg-white px-1.5 py-0.5 font-mono text-[10px] font-medium text-slate-400 shadow-2xs">
              ⌘K
            </kbd>
          </div>
        </form>
        ...
        <div className="flex items-center gap-3">
            ...
            {isAuthenticated && user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-1.5 pr-3 transition hover:border-slate-300 hover:bg-slate-50 shadow-2xs cursor-pointer"
                >
                  <img
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-1.5 w-48 rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg z-20">
                      <Link
                        ...
                        className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100 transition"
                      >
                      ...
                      <button
                        ...
                        className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 transition text-left cursor-pointer"
                      >
              ...
              <div className="flex items-center gap-2">
                <NavLink
                  to="/login"
                  className="rounded-lg border border-slate-200 px-3.5 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-100 hover:text-slate-900"
                >
                  Sign In
                </NavLink>
                <NavLink
                  to="/register"
                  className="rounded-lg bg-slate-900 px-3.5 py-1.5 text-xs font-semibold text-white transition hover:bg-slate-800 shadow-sm"
                >
                  Register
                </NavLink>
              </div>
          ...
          <button
            ...
            className="inline-flex items-center rounded-lg border border-slate-200 bg-white p-2 text-slate-700 shadow-2xs transition hover:bg-slate-50 md:hidden cursor-pointer"
          >
          ...
          >
            <div className="mx-auto max-w-[1536px] px-4 py-4 space-y-3">
              ...
              <form onSubmit={handleSearchSubmit} className="sm:hidden">
                <div className="relative flex items-center rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-slate-800">
                  <HiSearch className="h-4 w-4 text-slate-400 mr-2 flex-shrink-0" />
                  <input
                    type="search"
                    value={searchQuery}
              ...
              {isAuthenticated && user ? (
                ...
                <div className="flex flex-col gap-2 pt-1">
                  <Link
                    to="/login"
                    onClick={closeMobileMenu}
                    className="w-full text-center rounded-lg border border-slate-200 px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50"
                  >
                    Sign In
                  </Link>
                  <Link
                    to="/register"
                    onClick={closeMobileMenu}
                    className="w-full text-center rounded-lg bg-slate-900 px-4 py-2.5 text-xs font-semibold text-white hover:bg-slate-800 shadow-sm"
                  >
                    Register
                  </Link>
                </div>
