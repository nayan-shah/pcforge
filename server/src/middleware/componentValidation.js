...
const validationRules = [
  ...
  body('prices.*.store').optional({ values: 'falsy' }).trim().notEmpty().withMessage('Each price needs a store name.'),
  body('prices.*.storeName').optional({ values: 'falsy' }).trim().notEmpty().withMessage('Each price needs a store name.'),
  ...
  body('prices.*.price').optional({ values: 'null' }).isFloat({ min: 0 }).withMessage('Price must be non-negative.'),
  body('prices.*.currentPrice').optional({ values: 'null' }).isFloat({ min: 0 }).withMessage('Price must be non-negative.'),
  body('prices.*.currency').if(body('prices').exists()).isLength({ min: 3, max: 3 }).withMessage('Currency must be a 3-letter code.'),
  body('prices.*.inStock').optional().isBoolean().withMessage('inStock must be a boolean.'),
